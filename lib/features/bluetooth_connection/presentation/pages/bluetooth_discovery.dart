...
import '../bloc/bluetooth_state.dart' as ble_state;
...
class _DeviceScanScreenState extends State<DeviceScanScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocConsumer<BluetoothBloc, ble_state.BluetoothState>(
        listener: (context, state) {
          if (state is ble_state.BluetoothErrorState) {
            ScaffoldMessenger.of(context).showSnackBar(
        ...
        builder: (context, state) {
          if (state is ScanningState) {
          ...
          } else if (state is DevicesFoundState) {
            ...
            WidgetsBinding.instance.addPostFrameCallback((_) {
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (_) => WiFiSelectionScreen(device: state.device),
                ),
              );
            });
            return const Center(
