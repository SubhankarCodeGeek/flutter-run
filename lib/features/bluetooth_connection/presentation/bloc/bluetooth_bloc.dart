// bluetooth_bloc.dart
import 'dart:async'; // For Completer, StreamSubscription, Future, Duration, TimeoutException
import 'dart:convert'; // For jsonEncode, base64Encode, utf8

import 'package:cloud_firestore/cloud_firestore.dart'; // For FirebaseFirestore, Timestamp
import 'package:flutter_bloc/flutter_bloc.dart'; // For Bloc, Emitter
import 'package:flutter_blue_plus/flutter_blue_plus.dart'; // For BluetoothDevice, ScanResult, BluetoothCharacteristic, Guid, etc.
...
class BluetoothBloc extends Bloc<BluetoothEvent, BluetoothState> {
  ...
  Future<void> _onStartBleScan(
    ...
    try {
      ...
      final Set<DeviceIdentifier> discoveredDeviceIds = {}; // To avoid duplicate device entries
      ...
      scanResultsSubscription = FlutterBluePlus.scanResults.listen((results) {
        for (ScanResult r in results) {
          ...
          if (r.device.platformName.isNotEmpty && !discoveredDeviceIds.contains(r.device.remoteId)) {
      ...
      await scanResultsSubscription?.cancel(); // Cancel the subscription to scan results

      if (foundDevices.isEmpty) {
        // print("No BLE devices found");
        emit(const BluetoothDevicesFound([])); // Emit empty list if no devices are found
      } else {
        emit(BluetoothDevicesFound(List.from(foundDevices))); // Emit the list of found devices
      }
    } catch (e) {
  ...
  Future<void> _onConnectToDevice(
    ...
    try {
      ...
      _connectionStateSubscription = device.connectionState.listen((BluetoothConnectionState connectionState) {
        ...
        if (connectionState == BluetoothConnectionState.disconnected) {
          ...
          if (isRelevantState && !(this.state is BluetoothProvisioningSuccess)) {
      ...
      for (final service in services) {
          ...
          for (final characteristic in service.characteristics) {
            ...
            if (characteristic.uuid == _provisioningCharacteristicUuid) {
              ...
              if (canWrite && canNotify) {
                ...
                _charValueStream = characteristic.onValueReceived; // Stream for characteristic value changes
        ...
        if (_provisioningCharacteristic != null) break; // Service and characteristic found
      }
      ...
      if (_provisioningCharacteristic == null) {
        ...
        throw Exception('Provisioning characteristic not found. Ensure UUIDs match and it supports Write & Notify/Indicate.');
      }

      emit(BluetoothReadyForProvisioning(device)); // Indicate device is ready for provisioning
    } catch (e) {
      // print('Connection or service discovery failed: $e');
      await event.device.disconnect().catchError((_) {}); // Attempt to clean up by disconnecting
      emit(BluetoothError('Connection/Service Discovery failed: ${e.toString()}'));
    }
  }
  ...
  Future<void> _onSendWifiCredentials(
    ...
    if (_provisioningCharacteristic == null || _charValueStream == null) {
      emit(const BluetoothProvisioningFailure('Characteristic not available. Please reconnect.'));
      return;
    }
    emit(const BluetoothSendingWifiCredentials()); // Indicate credentials are being sent

    try {
      // Prepare payload (SSID and password)
      final payload = jsonEncode({'ssid': event.ssid, 'password': event.password});
      final base64Payload = base64Encode(utf8.encode(payload)); // Example encoding
      ...
      bool canWriteWithResponse = _provisioningCharacteristic!.properties.write; // Check if acknowledged write is supported
      await _provisioningCharacteristic!.write(bytesToSend, withoutResponse: !canWriteWithResponse);
      // print('Wi-Fi credentials sent to ${event.device.remoteId}. SSID: ${event.ssid}');

      emit(const BluetoothAwaitingProvisioningConfirmation()); // Indicate waiting for device response
      ...
      final response = await _charValueStream!.first.timeout(
        const Duration(seconds: 30), // Timeout for device to process and respond
        onTimeout: () {
          // print("Timeout waiting for provisioning confirmation from device.");
          throw TimeoutException('Device did not respond with provisioning status in time.');
        },
      );
      ...
      final String status = utf8.decode(response, allowMalformed: true); // Decode response
      // print('Received provisioning status from device: $status');

      // Check device status response
      if (status.toUpperCase().startsWith('SUCCESS') || status.toUpperCase() == 'OK') {
        // Provisioning successful
        final prefs = await SharedPreferences.getInstance();
        await prefs.setBool('provisioned_${event.device.remoteId.str}', true); // Mark device as provisioned
        ...
        emit(BluetoothProvisioningSuccess(event.device, message: "Successfully provisioned: $status"));
      } else {
  ...
  Future<void> _onDisconnect(
    ...
    try {
      ...
      if (!(state is BluetoothInitial || state is BluetoothProvisioningSuccess)) {
